...
export default function QuestionnairePage() {
  ...
  const submitAnswers = async () => {
    ...
    // Pastikan user login dulu (anonymous login kalau belum)
    const auth = getAuth();
    if (!auth.currentUser) {
    ...
    // Simpan ke Firestore
    await addDoc(collection(db, "riasecResults"), {
      userId,
        ...
        <div className="flex items-center justify-center space-x-4 text-sm">
          ...
          <span
            className={`px-3 py-1 rounded-full font-medium ${
              questions[currentQuestion].type === "R"
                ? "bg-red-100 text-red-700"
                : questions[currentQuestion].type === "I"
                ? "bg-blue-100 text-blue-700"
                : questions[currentQuestion].type === "A"
                ? "bg-purple-100 text-purple-700"
                : questions[currentQuestion].type === "S"
                ? "bg-green-100 text-green-700"
                : questions[currentQuestion].type === "E"
